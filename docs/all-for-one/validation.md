...
Execution is user-initiated and does not add a model-callable tool. `/validate run` and unambiguous category commands require a trusted project, a dialog-capable UI, and confirmation showing the exact program, resolved executable path, argument array, working directory, source, and confidence. The host executable is resolved before approval, workspace-local executable shadows are rejected, and the same canonical executable identity is required immediately before process creation. The process is spawned directly without a shell, with bounded stdout and stderr capture and a two-minute timeout. Inferred commands receive a stronger warning. Ambiguous categories display choices instead of selecting silently. Discovery is re-read before approval and again before process creation; if repository validation configuration changes, execution fails closed and requires a refreshed selection.
