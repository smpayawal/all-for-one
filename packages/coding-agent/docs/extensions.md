...
Extensions can override built-in tools (`read`, `bash`, `edit`, `write`, `apply_patch`, `grep`, `find`, `ls`) by registering a tool with the same name. Interactive mode displays a warning when this happens.
...
- [apply-patch.ts](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/src/core/tools/apply-patch.ts) - `ApplyPatchToolDetails`
...
const darkTheme = ctx.ui.getTheme("dark");  // Load without switching
const result = ctx.ui.setTheme("dark");  // Switch by name
if (!result.success) {
	ctx.ui.notify(`Failed: ${result.error}`, "error");
}
ctx.ui.setTheme(darkTheme!);  // Or switch by Theme object
