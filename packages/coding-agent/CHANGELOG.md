...
- Added default `apply_patch` and `changes` tools for preflighted multi-file edits with best-effort rollback and bounded Git inspection, plus non-executing validation-command guidance discovered from root project manifests.
- Added a responsive All-For-One Focus Rail backed by centralized display identity that surfaces verified session activity and loaded context on wide terminals while preserving the vanilla full-width fallback.
