...
Then just talk to pi. By default, pi uses the model profile to select the four-tool mutation surface: native models get `read`, `bash`, `edit`, and `write`; patch-oriented profiles get `apply_patch` instead of `edit`. Use `--tool-profile native`, `--tool-profile patch`, or `--tool-profile full` to fix the surface explicitly. Add capabilities via [skills](#skills), [prompt templates](#prompt-templates), [extensions](#extensions), or [pi packages](#pi-packages).
...
| `--tool-profile <profile>`, `-tp <profile>` | Select the auto, native, patch, or full built-in profile |
